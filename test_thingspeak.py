...
url = "https://thingspeak.com/channels/public?page=1"

headers = {"Authorization": f"Bearer {API_KEY}"}
payload = {
    "url": url,
    "pageOptions": {"onlyMainContent": False, "waitFor": 2000}
}
print("Fetching content...")
try:
    response = requests.post("https://api.firecrawl.dev/v0/scrape", json=payload, headers=headers)
    res_json = response.json()
    with open('ts_response.json', 'w', encoding='utf-8') as f:
        json.dump(res_json, f)

    if "data" in res_json and "markdown" in res_json["data"]:
        with open('ts_markdown.txt', 'w', encoding='utf-8') as f:
            f.write(res_json["data"]["markdown"])
        print("Markdown saved to ts_markdown.txt")
    else:
